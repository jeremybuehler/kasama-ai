 ...
 * Displays available OAuth providers for authentication
 */
...
const OAuthProviders: React.FC<OAuthProvidersProps> = ({
  ...
  const providers: OAuthProvider[] = [
    ...
    },
    {
      ...
      color: 'border-gray-800 hover:bg-gray-100 text-gray-900'
    },
    {
      name: 'Microsoft',
      provider: 'microsoft',
      ...
      color: 'border-blue-500 hover:bg-blue-50 text-blue-700'
    },
    {
      name: 'Apple',
      provider: 'apple',
      ...
      color: 'border-black hover:bg-gray-100 text-black'
    }
  ];
  ...
  const handleProviderClick = async (provider: OAuthProvider) => {
    ...
    try {
      ...
      // Trigger OAuth flow
      await authService.signInWithOAuth(provider.provider);
    } catch (error) {
      console.error(`OAuth ${provider.name} error:`, error);
    } finally {
        ...
        {enabledProviders.map((provider) => (
          <button
            ...
            className={`
              ...
              ${provider.color}
              ${loadingProvider === provider.provider ? 'opacity-75' : ''}
            `}
          >
